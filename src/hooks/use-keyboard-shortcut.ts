...
export const useKeyboardShortcut = <T extends HTMLElement>(
  shortcutKeys: string[],
  ...
  const keydownListener = useCallback(
    (keydownEvent: KeyboardEvent): boolean | undefined => {
      ...
      if (
        ...
        // @ts-expect-error -- TS doesn't know about the `tagName` property on `target`.
  ...
  // useEffects
  useEffect(() => {
    (targetElement as T).addEventListener('keydown', keydownListener);
    (targetElement as T).addEventListener('keyup', keyupListener);
    return () => {
      (targetElement as T).removeEventListener('keydown', keydownListener);
      (targetElement as T).removeEventListener('keyup', keyupListener);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [targetElement, keydownListener, keyupListener, shortcutKeysId]);
